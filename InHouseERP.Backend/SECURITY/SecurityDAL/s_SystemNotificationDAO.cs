...
namespace SecurityDAL
{
    public class s_SystemNotificationDAO
    {
        ...
        public Int64 UpdateSystemMaintenance(s_SystemNotification SystemNotification)
        {
            Int64 ret = 0;
            try
            {
                ...
                dbExecutor.ManageTransaction(TransactionType.Open);
                ret = dbExecutor.ExecuteScalar64(true, CommandType.StoredProcedure, "s_SystemNotification_Update",
                    colparameters, true);
                dbExecutor.ManageTransaction(TransactionType.Commit);
            }
            catch (DBConcurrencyException except)
